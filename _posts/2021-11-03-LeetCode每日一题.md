...
class Solution {
    ...
    public int checkWays(int[][] pairs) {
        int MAXNUM = 501;
        // 记录各个数字出现次数，因为The elements in pairs are unique，所以可以表示这个数字节点的度
        int[] cnts = new int[MAXNUM];
        // 记录某节点的父节点
        int[] father = new int[MAXNUM];
        // 关系图，类似邻接矩阵
        boolean[][] graph = new boolean[MAXNUM][MAXNUM];
        // 用于去重，拿到所有数字
        ...
        int numsNum = set.size();
        // 数字个数-1还要比pair数多，是一定不能构成同一棵树把所有数字用上的，只能构成森林。
        if(pairs.length < numsNum - 1) return 0;

        // 将数字放入list以按照出现次数排序
        List<Integer> list = new ArrayList<>(set);
        // 出现次数多的在前，次数递减序
        Collections.sort(list, (x, y) -> cnts[y] - cnts[x]);
        ...
        for(int i = 1; i < numsNum; ++i) {
            ...
            // 用于记录节点是否插入
            boolean judge = false;
            // i之前的节点已经插入完毕（i从1开始，默认root已插入），遍历以检查当前节点与其之前的节点是否有关系（找爸爸）
            for(int j = i - 1; j >= 0 && !judge; --j) {
                // 之前的节点（已经插入了的节点）
                int  y = list.get(j);
                // 存在关系
                if(graph[x][y]) {
                    // 记录当前节点的父亲为最先找到的这一个祖先
                    father[x] = y;
                    // 使内循环结束
                    judge = true;
                }
            }
            // 如果遍历一圈当前节点与前面的节点没有关系，说明至少需要构成两棵树，或者说明存在孤岛
            // 不能构造一棵满足The tree consists of nodes whose values appeared in pairs的树
            if(!judge) return 0;
        }

        // counter记录所有的路径
        int counter = 0, ans = 1;
        // 遍历set检查构造的树是否唯一（默认ans = 1，不唯一修改为2）
        for(int i : set) {
            int j = i;
            // 向上查，直到根节点
            while(father[j] != -1) {
                // 如果当前节点与它父亲或父亲的祖先没有关系，不成立（因为父亲的祖先应当也是自己的祖先）
                if(!graph[i][father[j]]) return 0;
                // 如果当前节点与它某个祖先的度相同，他们的位置可以互换，说明这样的树不唯一 ans=2
                // 因为题目中的pair记录的x可以是y的祖先，y也可以是x的祖先
                if(cnts[i] == cnts[father[j]]) ans = 2;
                ++counter;
                // 沿着父亲的方向更新j
                j = father[j];
            }
        ...
        // 最后再检查一次路径数（边的数量）会不会小于pairs的数量
        // 应当是不可能比pairs的数量还小的，最小应该是counter == pairs.length
        // 排除最后一种无法构成树的情况，返回ans
        return counter < pairs.length ? 0 : ans;
    }
}
```
...
n个nodes，n - 1条边与中心结点连接，且是星形图，随便抓两组关系找都出现过的节点就ok了：
...
class Solution {
    public String removeKdigits(String num, int k) {
        ...
        while(index < len) {
            ...
            // 栈不空，栈顶元素大于当前元素且能保证出栈后剩余元素能够之前的count，出栈
            while(!stack.isEmpty() && stack.peekLast() > num.charAt(index) &&
                  stack.size() + len - index - 1 >= count) {
                stack.removeLast();
            }
            // 当前元素入栈比他大的已经出栈了，它入栈能保证递增）
            stack.addLast(num.charAt(index++));
        }

        if(stack.isEmpty()) return "0"; // 可不要
...
## [hard] 1675. Minimize Deviation in Array
 ...
 ![image-20220219132854334](https://s2.loli.net/2022/02/19/LoPNBmeOiEF4Sxf.png)
...
将所有数字变为偶数，之后只需做除法，使最大的数字不能被除以2为止，期间出现的最小的deviation则为答案

因为最大的数字已经是奇数了，比它还小的偶数再除以2也只会让它们之间差距越来越大。

使用`treeSet`这种数据结构，既能去重，又能方便地拿到最大值最小值
...
class Solution {
    public int minimumDeviation(int[] nums) {
		...
		// then, only do division to make huge number smaller
		// calculate the deviation
        int deviation = set.last() - set.first();
		// maxNum / 2 while maxNum is even
        while(set.last() % 2 == 0) {
            set.add(set.last() / 2);
            set.remove(set.last());
            deviation = deviation < set.last() - set.first() ?
                deviation : set.last() - set.first();
        }
		// when maxNum is even (cannot do division), the smallest deviation is the answer
        return deviation;
    }
}
```
...
class Solution {
    public int removeCoveredIntervals(int[][] intervals) {
        ...
        for(int[] array : intervals) {
            // 仅当当前这组值左边和右边分别大于l与r时，这组不会被覆盖ans++
            if(array[0] > l && array[1] > r) {
                // 记录左边界
                l = array[0];
                ++ans;
            }
            // 记录右边界
            r = r > array[1] ? r : array[1];
        }
        return ans;
    }
}
...
很好理解，但不好实现，知道最简单的方法是使用hashmap存储，但是关系对应的细节不太好想。
...
class Solution {
    ...
    // 用于建立map的key-value对应和遍历neighbors，dfs遍历图
    public Node cloneGraph(Node node) {
        // 终止条件
        if(node == null) return node;
        // 对于已经被克隆过的结点不做任何处理，返回它的克隆结点
        if(map.containsKey(node)) return map.get(node);
        // 克隆结点，key为原结点，value为新结点
        map.put(node, new Node(node.val));
        // 遍历neighbors
        for(Node neighbor : node.neighbors) {
            // 递归地将所有邻接结点的克隆放入当前结点的克隆的neighbors表里
